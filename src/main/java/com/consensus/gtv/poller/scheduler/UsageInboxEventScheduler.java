...
@Slf4j
@Component
public class UsageInboxEventScheduler extends AbstractEventScheduler<IspNewUsageEvent> {

    private static final String JOB_NAME = "usage inbox";
    private static final String JOB_LOCK_ID = "USAGE_INBOX_POLLER_JOB_LOCK";

    private final CoreDbRepository coreDbRepository;
    private final IspUsageMapper ispUsageMapper;

    public UsageInboxEventScheduler(IspDataPublishService ispDataPublishService, CoreDbRepository coreDbRepository,
    ...
    protected JobPollResult<IspNewUsageEvent> pollEvents(DbLockData dbLockData, int batchSize) {
        ...
        List<IspNewUsageEvent> events = coreDbRepository.getUsageInboxEvents(startTimestamp, endTimestamp, batchSize)
                .stream()
                .map(ispUsageMapper::toNewUsageEvent)
                .collect(toList());

        LOG.debug("Pooled 'IspNewUsageEvent' inbox events: {}", events);
